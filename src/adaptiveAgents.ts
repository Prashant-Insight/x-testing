import { AdaptivePlan, Artifact, BusinessReportDraftPayload, DatabricksWorkspaceConnection, Draft, ExternalConnectionDraftPayload, MetadataDraftPayload, NotebookDraftPayload, OrchestrationMode, PlanReviewPayload, PowerBIDraftPayload, PowerBIColumn, UserIntent } from './domain';

export interface PowerBIBuildContext { businessReport?: BusinessReportDraftPayload; cleanTable: { catalog: string; schema: string; table: string }; workspaceHost: string; }
export interface AgentExecutionResult<TPayload> { draft?: Draft<TPayload>; artifacts: Artifact[]; }

const order: UserIntent['capabilities'] = ['business-understanding', 'synthetic-data', 'metadata', 'databricks-assets', 'powerbi-assets', 'external-connection'];

function now(): string { return '2026-01-01T00:00:00.000Z'; }
function slug(input: string): string { return input.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 40) || 'lakepilot'; }
function id(prefix: string, input: string): string { let hash = 0; for (const ch of input) { hash = ((hash << 5) - hash + ch.charCodeAt(0)) | 0; } return `${prefix}-${Math.abs(hash)}`; }
function draft<TPayload>(draftType: Draft<TPayload>['draftType'], title: string, agentName: string, payload: TPayload): Draft<TPayload> { return { draftId: id(draftType, `${title}:${JSON.stringify(payload)}`), draftType, approvalRequired: true, title, agentName, status: 'pending', createdAt: now(), payload }; }
function artifact(type: string, title: string, content: string): Artifact { return { artifactId: id(type, `${title}:${content}`), type, title, content, createdAt: now() }; }
function has(request: string, regex: RegExp): boolean { return regex.test(request.toLowerCase()); }

export class IntentPlannerAgent {
  public createPlan(request: string, workspace: DatabricksWorkspaceConnection, mode: OrchestrationMode): AdaptivePlan {
    const capabilities = new Set<UserIntent['capabilities'][number]>(['business-understanding']);
    if (has(request, /notebook|delta|clean|ingest|json|csv|github/)) capabilities.add('databricks-assets');
    if (has(request, /power\s*bi|powerbi|semantic|report/)) capabilities.add('powerbi-assets');
    if (has(request, /github|api|url|source/)) capabilities.add('external-connection');
    if (has(request, /metadata|watermark|incremental|primary key/)) capabilities.add('metadata');
    if (has(request, /synthetic|sample data/)) capabilities.add('synthetic-data');
  ...
  public createDraft(plan: AdaptivePlan): Draft<PlanReviewPayload> {
...
export class BusinessUnderstandingAgent {
  public execute(plan: AdaptivePlan): AgentExecutionResult<BusinessReportDraftPayload> {
    ...
    const payload: BusinessReportDraftPayload = {
...
export class AdaptiveDatabricksAgent {
  public execute(plan: AdaptivePlan): AgentExecutionResult<NotebookDraftPayload> {
    const scenario = slug(plan.intent.request);
    const payload: NotebookDraftPayload = {
      scenario,
      catalog: 'main',
...
export class JobPipelineAgent {
  public execute(plan: AdaptivePlan): AgentExecutionResult<NotebookDraftPayload> {
    const base = new AdaptiveDatabricksAgent().execute(plan).draft?.payload as NotebookDraftPayload;
...
export class AdaptiveMetadataAgent {
  public execute(): AgentExecutionResult<MetadataDraftPayload> {
    const payload: MetadataDraftPayload = { catalog: 'main', schema: 'lakepilot_demo', table: 'clean_report_source', primaryKeys: ['customer_id', 'event_date'], watermarks: ['event_date'], incrementalStrategy: 'merge-on-primary-key-and-watermark', columns: [{ name: 'quality_status', type: 'string', description: 'Row-level data quality state.' }] };
    return { draft: draft('metadata-grid', 'Review metadata grid', 'AdaptiveMetadataAgent', payload), artifacts: [] };
  }
}

export class ExternalConnectionAgent {
  public execute(plan: AdaptivePlan): AgentExecutionResult<ExternalConnectionDraftPayload> {
    const payload: ExternalConnectionDraftPayload = { sourceType: has(plan.intent.request, /github/) ? 'github' : 'external-source', sourceHint: plan.intent.request, connectionName: 'lakepilot_source_connection', secretRequirements: ['Store tokens in VS Code SecretStorage or Databricks secrets before use.'], ingestionNotes: ['No connection is opened by the core engine.', 'Review and approve credentials outside LakePilot drafts.'], configJson: JSON.stringify({ connection: 'lakepilot_source_connection', mode: 'review-only' }, null, 2) };
    return { draft: draft('external-connection-review', 'Review external connection draft', 'ExternalConnectionAgent', payload), artifacts: [] };
  }
}

export class AdaptiveSyntheticDataAgent {
  public execute(): AgentExecutionResult<never> {
    return { artifacts: [artifact('csv-preview', 'Synthetic CSV preview', 'event_date,customer_id,amount,category\n2026-01-01,C-1,42.00,A')] };
  }
}

export class AdaptivePowerBIAgent {
  public execute(plan: AdaptivePlan, context: PowerBIBuildContext): AgentExecutionResult<PowerBIDraftPayload> {
    ...
    const columns: PowerBIColumn[] = [...baseColumns, { name: 'quality_status', dataType: 'string', businessMeaning: 'Generated quality classification for each row.' }];
    ...
    const payload: PowerBIDraftPayload = {
...
function draftTypeFor(capability: UserIntent['capabilities'][number]): Draft['draftType'] | undefined {
  switch (capability) {
...
function titleFor(capability: UserIntent['capabilities'][number]): string { return capability.split('-').map(part => part[0].toUpperCase() + part.slice(1)).join(' '); }
function rationaleFor(capability: UserIntent['capabilities'][number], request: string): string { return `${capability} selected for request: ${request.slice(0, 120)}`; }
function mapType(type: string): PowerBIColumn['dataType'] { if (/int|long/.test(type)) return 'int64'; if (/decimal|money|amount/.test(type)) return 'decimal'; if (/double|float/.test(type)) return 'double'; if (/date|time/.test(type)) return 'dateTime'; if (/bool/.test(type)) return 'boolean'; return 'string'; }
function buildTmdl(tableName: string, columns: PowerBIColumn[], measures: string[], powerQueryM: string): string {
